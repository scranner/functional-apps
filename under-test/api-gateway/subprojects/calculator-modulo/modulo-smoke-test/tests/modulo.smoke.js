...
describe('When a number of requests are made to the service', () => {
  ...
  describe('Which are valid', () => {
    it('3 % 2 = 1', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: 3, y: 2 } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('1');
    });

    it('"10" % "5" = 0', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: '10', y: '5' } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('0');
    });

    it('"2e4" % "4" = 0', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: '2e4', y: '4' } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('0');
    });
