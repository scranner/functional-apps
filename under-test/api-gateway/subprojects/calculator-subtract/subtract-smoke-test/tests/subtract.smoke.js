...
describe('When a number of requests are made to the service', () => {
  ...
  describe('Which are valid', () => {
    it('4 - 2 = 2', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: 4, y: 2 } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('2');
    });

    it('"10" - 2 = 8', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: '10', y: 2 } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('8');
    });

    it('"1e2" - 2 = 98', async () => {
      const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: '1e2', y: 2 } });
      timings.push(result.timing);
      expect(result.result.data.result).toBe('98');
    });

    describe('Which are invalid', () => {
      it('"test" - 2 = Invalid Parameters, 400', async () => {
        const result = await timeFunction(axios.get, `http://${API_UNDER_TEST}`, { params: { x: 'test', y: 2 } });
        timings.push(result.timing);
